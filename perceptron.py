...
class Tree_Perceptron:
    ...
    def run_ai(
        ...
        theta = self.logistic_regression(np.array([ai]), yp,
                                         self.learning_rate, 3, theta)
        for j in range(self.n_tree + 1):
            self.parameters[class_i][j] = theta[0][j]

    def yhat(  # predict based on current parameters
            self,
            theta,
            X,
            n,
    ...
    def make_param_table(self, n_class, n_tree):
        ...
        param_table = [[1 / n_class for x in range(n_tree + 1)]
